...
class BM25(object):
    ...
    def __init__(self, docs):
        self._D = len(docs)
        self._dl = torch.tensor([len(doc) for doc in tqdm(docs)])
        self._avg_dl = self._dl.sum().float() / self._D
        self._words = set([word.lower() for doc in tqdm(docs) for word in doc])
        self._word2id = {word: i for i, word in tqdm(enumerate(self._words))}
...
if __name__ == '__main__':
    ...
    for ans in tqdm(answer_list):
        answer_list_cut.append(list(jieba.cut(ans)))
    s = BM25(answer_list_cut)
    print('--stage4')
    all_query_ids = []
    all_answers_ids = []
    for query_id, answers_id in tqdm(query2answer_id.items()):
